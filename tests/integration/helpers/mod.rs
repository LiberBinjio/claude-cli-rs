...
use claude_query::{QueryEngine, ToolSet};
...
pub async fn create_test_engine(mock: &MockAnthropicApi, cwd: PathBuf) -> QueryEngine {
    ...
    // Convert ToolRegistry → ToolSet for QueryEngine
    let mut tool_set = ToolSet::new();
    for tool in tool_registry.all() {
        tool_set.register(tool.clone());
    }
    let tool_set = Arc::new(tool_set);

    let mut cmd_registry = CommandRegistry::new();
    register_builtins(&mut cmd_registry);
    let cmd_registry = Arc::new(cmd_registry);

    QueryEngine::new(api_client, tool_set, cmd_registry, cwd)
}
